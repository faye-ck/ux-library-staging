...
**Step A — Inventory Figma components**

From the design context already extracted by the figma-use reading protocol:
1. List every distinct component, pattern, and interactive element visible in the frames
2. For each, find the nearest match in the `mui-design` Component Catalog (Section 1) — use the same 6-category structure: Inputs, Data Display, Feedback, Surfaces, Navigation, Utilities
3. Record: Figma component name → MUI component name → category → recommended variant
4. For components with no MUI match: classify as "defined here" — flag for promotion if the same pattern appears in 3+ product specs

**Step B — Map Figma styles to MUI tokens**

From every color fill, spacing value, typography style, shadow, and corner radius found:
1. Apply the Figma→MUI Token Mapping table from `mui-design` (Section 4)
2. Cross-reference the product's `_theme.md` — if a token has a product-level alias (e.g. `--color-primary`), record that instead of the raw MUI path
3. Flag any raw hex value with no token match as an Open Item (inconsistency)
4. All spacing must resolve to `theme.spacing(n)` — never document pixel values directly

**Step C — Produce the MUI inventory**

Output a structured block that feeds directly into the spec:

```
── MUI Component Mapping ─────────────────────────────
[Figma component]  →  [MUI component]  ([category])
  variant: [variant]  ·  source: global/components/

[Component with no MUI match]  →  defined here
  ⚠ Promote if used in 3+ product specs

── MUI Token Mapping ─────────────────────────────────
[Figma style / fill]  →  [MUI token path]
  e.g.  Primary fill  →  palette.primary.main

── Flags ─────────────────────────────────────────────
[Raw hex / undefined token]  →  Open Item: no token match
```

This output is consumed directly by `document-feature-spec` (Step 1) and `add-platform-delta` (Step 1) to populate the "Components Used" and "Color Tokens Used" sections without any manual mapping.
...
| Script | Purpose |
|--------|---------|
| [inspectFileStructure.js](../figma-generate-library/scripts/inspectFileStructure.js) | Discover all pages, components, variables, styles; returns full inventory |
| [createVariableCollection.js](../figma-generate-library/scripts/createVariableCollection.js) | Create a named collection with modes; returns `{collectionId, modeIds}` |
| [createSemanticTokens.js](../figma-generate-library/scripts/createSemanticTokens.js) | Create aliased semantic variables from a token map |
| [createComponentWithVariants.js](../figma-generate-library/scripts/createComponentWithVariants.js) | Build a component set from a variant matrix; handles grid layout |
| [bindVariablesToComponent.js](../figma-generate-library/scripts/bindVariablesToComponent.js) | Bind design tokens to all component visual properties |
| [createDocumentationPage.js](../figma-generate-library/scripts/createDocumentationPage.js) | Create a page with title + description + section structure |
| [validateCreation.js](../figma-generate-library/scripts/validateCreation.js) | Verify created nodes match expected counts, names, structure |
| [cleanupOrphans.js](../figma-generate-library/scripts/cleanupOrphans.js) | Remove orphaned nodes by name convention or state ledger IDs |
| [rehydrateState.js](../figma-generate-library/scripts/rehydrateState.js) | Scan file for all pages, components, variables by name; returns full `{key → nodeId}` map for state reconstruction |
