...
### What counts as a product/feature-specific rule

A rule belongs in the product config when the Figma reveals a design decision that:
- **Overrides a global MUI default** — e.g. border-radius 8px instead of MUI's 4px, elevation 0 instead of shadows[1]
- **Uses a custom variant** — e.g. a Button with trailing icon + count badge that isn't in the global Button spec
- **Applies a product token override** — e.g. primary color is brand teal, secondary is a custom orange not in MUI's palette
- **Establishes a unique interaction pattern** — e.g. auto-save on blur, drag-to-reorder, swipe-to-dismiss on mobile
- **Enforces feature-specific layout constraints** — e.g. "modal must always be bottom-sheet on mobile", "table always dense-size in this feature"
...
| Scope | Output target |
|-------|--------------|
| Applies to all features in this product | `[product]/config/_components.md` or `_guidelines.md` — append new section |
| Applies only to this feature | Inline **Design Rules** section at end of `feature.md` and `feature.html` |
...
3. **Classify each delta** — product-wide, feature-specific, or platform-specific
4. **Write only new rules** — append a new section with today's date; never overwrite existing config content
5. **Cross-reference global** — for every rule, confirm it doesn't already exist in `global/_guidelines.md` or `global/components/`

**Rule format for product config:**
```markdown
## [Feature or Component Name] — [YYYY-MM-DD]
Context: [which feature or platform this applies to]
- [Actionable rule referencing MUI token or component names]
  Figma source: [Figma component or frame name]
- ...
```

**Rule format for inline spec section:**
```markdown
### Design Rules
- [Rule] — deviates from [MUI default] because [reason from Figma annotation or inferred]
```
